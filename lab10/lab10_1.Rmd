...
life_history %>% 
  ...
  geom_col()+
  coord_flip()+
  scale_y_log10() # helps deal with very large differences
```
...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass=mean(mass, na.rm=T)) %>% 
  ggplot(aes(x=reorder(order, mean_mass), y=mean_mass))+ # reorder helps order bars
  geom_col()+
  coord_flip()+
  scale_y_log10()
```
...
```{r}
elephants <- read.csv(here("lab10","data","ElephantsMF.csv")) %>% clean_names()
```
...
elephants %>% 
  ...
  geom_point() + 
  geom_smooth(method=lm, se=F)+
  labs(title="Elephant Age vs. Height", #labs stands for labels
       x="Age",
       y="Height")
```
...
elephants %>% 
  ...
  labs(title="Elephant Age vs. Height",
       x="Age",
       y="Height")+
  theme(plot.title=element_text(size=rel(1), hjust=.5,  face="bold"), axis.text = element_text(size = 8), axis.title = element_text(size=10)) #h just is horizzontal adjust
```
...
```{r}
elephants %>% 
  ggplot(aes(x=sex, fill=sex))+ #fill means bars are color coded by sex. Can use fill to add variable not acitvely beinf plottted
  geom_bar()
```
...
```{r}
life_history %>% 
  ggplot(aes(x=gestation, y=log10(mass), size=mass))+ #size proportional to mass
  geom_point(na.rm=T)
```
...
```{r}
homerange <- read.csv(here("lab10","data","Tamburelloetal_HomeRangeDatabase.csv"))
glimpse(homerange)
```
...
homerange %>%  
  ...
  geom_bar()+
  labs(title = "Count By Thermoregulation", x = "Thermoregulation", y="Number of animals")+
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
```
...
homerange %>% 
  ...
  geom_point()+
  labs(title = "Mass vs Homerange", x = "Log 10 Mass", y="Log10.hra")+
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
```
