...
- Woodland has the highest diversity in the data
...
- Skylark, aka, Alauda arvensis has the smallest winter range
...
```
- Sooty Shearwater, aka, Puffinus griseus has the largest winter range
...
- The only exception is Common Eider, aka,	Somateria mollissima.
...
- Ducks that migrate longer distances tend to have lower average body mass as compared to other migratory strategies. Birds that are residents have the highest average body mass. The general trend longer travel distances tend to have a lower average body mass than shorter distances travelled.
...
- As seen below and from Problem 10, there are species that do not have any data for population size in the data frame. As a result, using solely population size, their conservation status cannot be determined and require additional information to make that classification.
...
- The question explored below is which bird family has the highest average body mass. As shown below, the corresponding bird family is Meleagrididae.
...
```
## # A tibble: 74 × 2
##    family            mean_body_mass
##    <chr>                      <dbl>
##  1 Meleagrididae               3.76
##  2 Gruidae                     3.7 
##  3 Pelecanidae                 3.64
##  4 Gaviidae                    3.48
##  5 Cathartidae                 3.46
