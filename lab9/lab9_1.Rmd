...
homerange %>% 
  ...
  ggplot(aes(x=common.name, y=log10.mass))+ #can flip x and y
  geom_col() #if specifying y. x must be categorical
```
...
homerange %>% 
  filter(class == "mammalia")%>% 
  count(trophic.guild) %>% 
  ggplot(mapping = aes(x=trophic.guild, y=n)) + 
  geom_col()
```
